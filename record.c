...
static struct record_head *record_find(unsigned long addr)
{
	struct list_head *p;
...
static int record_add(unsigned long addr)
{
	int ret;
	if (record_find(addr)) {
		...
		struct record_head *new =
			kmalloc(sizeof(struct record_head), GFP_KERNEL);
		new->addr = addr;
		new->count = 0;
...
static int record_remove(struct record_head *r)
{
	LOCK();
	list_del(&r->list);
	UNLOCK();
...
static int record_remove_by_addr(unsigned long addr)
{
...
static int handler_pre(struct kprobe *kprobe, struct pt_regs *_)
{
...
void set_kprobe(struct kprobe *kp, unsigned long addr)
{
...
static int register_func_by_name(const char *name)
{
...
static int deregister_func_by_name(const char *name)
{
	unsigned long addr = kallsyms_lookup_name(name);
	if (!addr) {
		pr_warning("%s: %s is not a valid kernel name\n",
